#!/bin/bash
#SBATCH --job-name=steer-aggregate
#SBATCH --output=logs/steer-aggregate-%j.log
...
# Override any of these via `sbatch --export=ALL,VAR=value jobs/steer_aggregate.sh`
TARGET="${TARGET:-block_angle}"
PROBE_DIR="${PROBE_DIR:-$STABLEWM_HOME/probes/$TARGET}"
LAYERS="${LAYERS:-layer_05 layer_07 layer_09 layer_11}"
DELTAS="${DELTAS:-0.1 0.2 0.3}"
TOLERANCE="${TOLERANCE:-0.02}"
NUM_PAIRS_PER_CONFIG="${NUM_PAIRS_PER_CONFIG:-5}"
OUTPUT_DIR="${OUTPUT_DIR:-$REPO/outputs/steering/aggregate}"
...
srun python -m interp_utils.steering.aggregate \
    --dataset "$DATASET" \
    ...
    --deltas $DELTAS \
    --tolerance "$TOLERANCE" \
    --num-pairs-per-config "$NUM_PAIRS_PER_CONFIG" \
